...
CzTROWallet::CzTROWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing ztro seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZTROSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZTROSeed_deprecated()) {
                    LogPrintf("%s: Updated zTRO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old ztro seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of ztro wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzTROWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZTROCount(nCountLastUsed);
    else if (!walletdb.ReadZTROCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzTROWallet::Lock()
{
    seedMaster = 0;
}

void CzTROWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzTROWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZTRO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzTROWallet::LoadMintPoolFromDB()
{
...
void CzTROWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzTROWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzTROWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->ztroTracker->HasPubcoinHash(pMint.first)) {
...
bool CzTROWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZTRO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to ztroTracker which also adds to database
    pwalletMain->ztroTracker->Add(dMint, true);
...
void CzTROWallet::SeedToZTRO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzTROWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzTROWallet::GenerateDeterministicZTRO(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzTROWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzTROWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
