...
// Copyright (c) 2016-2018 The Trunk developers
...
public:
    ...
    bool WriteZTROSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZTROSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZTROSeed_deprecated(uint256& seed);
    bool EraseZTROSeed();
    bool EraseZTROSeed_deprecated();

    bool WriteZTROCount(const uint32_t& nCount);
    bool ReadZTROCount(uint32_t& nCount);
