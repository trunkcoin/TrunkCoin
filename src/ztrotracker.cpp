...
CzTROTracker::CzTROTracker(std::string strWalletFile)
{
...
CzTROTracker::~CzTROTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzTROTracker::Init()
{
...
bool CzTROTracker::Archive(CMintMeta& meta)
{
...
bool CzTROTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzTROTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzTROTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzTROTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzTROTracker::GetSerialHashes()
{
...
CAmount CzTROTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzTROTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzTROTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzTROTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzTROTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzTROTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzTROTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzTROTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzTROTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzTROTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzTROTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzTROTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzTROTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzTROTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzTROTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzTROTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzTROTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dztro from DB\n", __func__, listDeterministicDB.size());
    }
