...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::TRO);
    ...
    if (!settings.contains("nAnonymizeTrunkAmount"))
        settings.setValue("nAnonymizeTrunkAmount", 1000);

    nAnonymizeTrunkAmount = settings.value("nAnonymizeTrunkAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTrunkAmount"))
        SoftSetArg("-anonymizetrunkamount", settings.value("nAnonymizeTrunkAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in trunk.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTrunkAmount:
            return QVariant(nAnonymizeTrunkAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTrunkAmount:
            nAnonymizeTrunkAmount = value.toInt();
            settings.setValue("nAnonymizeTrunkAmount", nAnonymizeTrunkAmount);
            emit anonymizeTrunkAmountChanged(nAnonymizeTrunkAmount);
            break;
        case CoinControlFeatures:
