...
#include "ui_ztrocontroldialog.h"
...
std::set<std::string> ZTroControlDialog::setSelectedMints;
std::set<CMintMeta> ZTroControlDialog::setMints;

ZTroControlDialog::ZTroControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZTroControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZTroControlDialog::~ZTroControlDialog()
{
    delete ui;
}

void ZTroControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTroControlDialog::updateList()
{
...
void ZTroControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTroControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZTro_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTroControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZTroControlDialog::GetSelectedMints()
{
...
void ZTroControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
