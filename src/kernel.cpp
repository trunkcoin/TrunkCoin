...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZTroStake(spend));
    } else {
        ...
        CTroStake* troInput = new CTroStake();
        troInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(troInput);
    }
