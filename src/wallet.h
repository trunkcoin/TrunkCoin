...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 TRO at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zTRO send
enum ZerocoinSpendStatus {
    ZTRO_SPEND_OKAY = 0,                            // No error
    ZTRO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZTRO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZTRO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZTRO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZTRO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZTRO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZTRO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZTRO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZTRO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZTRO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZTRO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZTRO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZTRO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZTRO_SPENT_USED_ZTRO = 14,                      // Coin has already been spend
    ZTRO_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZTRO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZTroBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZTROOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fztroAuto) const;
    ...
    */
    ...
    CzTROWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzTROTracker> ztroTracker;
    ...
    void setZWallet(CzTROWallet* zwallet)
    {
        zwalletMain = zwallet;
        ztroTracker = std::unique_ptr<CzTROTracker>(new CzTROTracker(strWalletFile));
    }

    CzTROWallet* getZWallet() { return zwalletMain; }
    ...
    /** zTRO reset */
    boost::signals2::signal<void()> NotifyzTROReset;
